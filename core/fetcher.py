...
def get_posts(username):
    ...
    try:
        ...
        if user_data.get("is_private"):
            colorPrint("The profile is Private. Fetching only collaborated posts (if available)...", LIGHTWHITE_EX)
        else:
            colorPrint("The profile is Public. Fetching only collaborated posts (if available)...", LIGHTWHITE_EX)
